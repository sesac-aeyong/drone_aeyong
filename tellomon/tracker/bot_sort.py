...
class STrack(BaseTrack):
    ...
    def _safe_ndivide(self, lv, rv, eps=1e-10):
    ...
    def update_features(self, feat, eps=1e-10):
        ...
        self._safe_ndivide(self.smooth_feat, self.smooth_feat)
...
class BoTSORT(object):
    ...
    def update(self, output_results, img, embeddings=None):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        if embeddings is not None:
            if len(embeddings) != len(dets):
                raise ValueError(f'Length of embeddings {len(embeddings)} doesn\'t match high-confidence detections {len(dets)}')
            features_keep = embeddings
        else:
            features_keep = None
